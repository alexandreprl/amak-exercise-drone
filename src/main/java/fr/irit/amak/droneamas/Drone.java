 ...
 */
public class Drone extends Agent<DroneAmas, World> {
	/**
	 ...
	 *
	 * @param amas   The AMAS the dorne belongs to
	 * @param startX The initial x coordinate of the drone
	 * @param startY The initial y coordinate of the drone
	 */
	public Drone(DroneAmas amas, int startX, int startY) {
		super(amas);
		dx = startX;
		dy = startY;
		drawable = new DrawableRectangle(amas.getGraphicsPanel(), dx * 10 - 400, dy * 10 - 300, 10, 10);
		drawable.setLayer(1);
	...
	protected void onAgentCycleEnd() {
		drawable.move(dx * 10 - 400, dy * 10 - 300);
	}
